...
PATH_TO_IMAGE_FOLDER = os.path.join(APP_ROOT, 'images')

class CameraUtility(object):
  def __init__(self):
    pass
  def generate_image_file(self, num, length):
    letters = "1234567890abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
    return 'image-' + ''.join( (random.choice(letters) + str(7 >> 1)) for i in range(length)) + str(num) + '.png'
...
cdef class Camera:
    cdef:
        ...
        CImageFormatConverter cImageFormatConverter_obj
        CPylonImage cPylonImage_obj
    ...
    def grab_images(self, int nr_images, unsigned int timeout=5000):
        ...
        cdef IImage* img
        cdef IReusableImage* iReusableImage_obj
        cdef EPixelType ePixelType_obj
        cdef CPylonImage cPylonImage_obj
        #cpdef Pylon::String_t file_name

        cdef EImageFileFormat eImageFileFormat_obj
        cdef CImagePersistenceOptions* cImagePersistenceOptions_obj
        #file_name = "picture.png"

        cImageFormatConverter_obj = new CImageFormatConverter()
        cImagePersistenceOptions_obj = new CImagePersistenceOptions()
        ...
        while self.camera.IsGrabbing():
            ...
            cImageFormatConverter_obj.OutputPixelFormat.SetValue(PixelType_RGB8packed)
            cImageFormatConverter_obj.Convert(cPylonImage_obj, <IImage&>ptr_grab_result) 
                   
            #cImagePersistenceOptions_obj.SetQuality(99) 
            #cPylonImage_obj.Save(ImageFileFormat_Png, <String_t &>file_name)
             
            img = <IImage*>&(cPylonImage_obj)
  
            image_obj = Image.frombuffer("RGB",(img.GetWidth(), img.GetHeight()), (<char*>img.GetBuffer())[:img.GetImageSize()]) 
