...
PATH_TO_SIMULATION_FILE = os.path.join(APP_ROOT, 'files','messages_for_simulation')
...
class CloudApis(Utility):
  ...
  def publish_message_to_subpub(self, project_id, registry_id, device_id, message_type, base_url, cloud_region, algorithm, private_key_file, message_data_type, file_data):
     ...
     Args:
       ...
       algorithm: algorithm used to create web token
       private_key_file: path to private key file 
       message_data_type: Three options "image_file", "image_string" or read message a from file limit to 1 message per line. The options string indicating image path, image string 
     ...
     """
     try:
       if(message_data_type == "image_file"):
         file_path = file_data
       ...
       elif(message_data_type == "image_string"):
         if(file_data):
            jwt_token = self.create_jwt(project_id, private_key_file, algorithm)
            payload = '{}'.format(file_data)
